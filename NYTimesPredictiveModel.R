...
nytimesCorpus = Corpus(VectorSource(c(nytimesDataset$combcol))) #each entry converted to document
...
#splitting the file to training and test dataset
set.seed(1234) #to pick rows randomly
...
#to check the percentage of response variable in training dataset
sum(nytimes.dfTrain$Popular == 'Y')/nrow(nytimes.dfTrain)

#applying neural net model
nnet.nytimes <- nnet(Popular ~ .,data=nytimes.dfTrain,size=5, decay=0.1, maxit=500)
